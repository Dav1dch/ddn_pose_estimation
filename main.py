...
def angle_axis_to_rotation_matrix(angle_axis):
    ...
    def angle_axis_to_rotation_matrix_rodrigues(angle_axis, theta2):
        ...
        ry = r[..., 1]  # b
        rz = r[..., 2]  # b
        r_skew = torch.zeros_like(
            r).unsqueeze(-1).repeat_interleave(3, dim=-1)  # bx3x3
        r_skew[..., 2, 1] = rx
        r_skew[..., 1, 2] = -rx
        r_skew[..., 0, 2] = ry
        r_skew[..., 2, 0] = -ry
        ...
        R = torch.eye(3, dtype=r.dtype, device=r.device).unsqueeze(0) \
            ...
            + (1.0 - theta.cos().unsqueeze(-1)) * \
            torch.matmul(r_skew, r_skew)  # bx3x3
        return R

    def angle_axis_to_rotation_matrix_taylor(angle_axis):
        rx, ry, rz = torch.chunk(angle_axis, 3, dim=-1)
        ones = torch.ones_like(rx)
        R = torch.cat([ones, -rz, ry, rz, ones, -rx, -
                      ry, rx, ones], dim=1).view(-1, 3, 3)
        return R
    ...
    eps = 1e-6
    if (theta2 > eps).all():
        rotation_matrix = angle_axis_to_rotation_matrix_rodrigues(
            angle_axis, theta2)
    else:
        ...
        for b in range(angle_axis.shape[0]):
            if theta2[b, ...] > eps:
                rotation_matrix[b, ...] = rotation_matrix_rodrigues[b:(
                    b+1), ...]
...
def J(p3d, p2d, y, k):
    projected_points = project_3d_2d(torch.as_tensor(
        p3d, dtype=torch.float32), y, torch.as_tensor(k, dtype=torch.float32))[:, :2]
    error = torch.sum((projected_points -
                       torch.as_tensor(p2d, dtype=torch.float32))**2, dim=-1)
