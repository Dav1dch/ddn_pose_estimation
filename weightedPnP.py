...
def weightedAngularReprojectionError(W, p2d, p3d, theta):
    ...
    # error = torch.einsum(
    #     "bmn,bmn->b", (W, 1.0 - torch.einsum("bmd,bnd->bmn", (p2d, p3dt)))
    # )
    # return error
    Kv = torch.tensor([[585.0, 585.0, 320.0, 240.0]]).cuda()
    # p2d_n = torch.nn.functional.normalize(p2d, dim=-1)
    p2d_n = p2d / torch.tensor([640, 480]).cuda()

    projectedPoint = geo.project_points_by_theta(p3d, theta, Kv)
    projectedPoint = projectedPoint / torch.tensor([640, 480]).cuda()
    squared_error = torch.sum((projectedPoint - p2d_n) ** 2, dim=-1)
...
def Dy(f, x, y):
    ...
    with torch.enable_grad():
        ...
        for i in range(DYf.size(-1)):  # [0,m)
            DYf_i = DYf[:, i]  # b
            DYYf[:, i : (i + 1), :] = (
                grad(DYf_i, y, grad_outputs=grad_outputs, create_graph=True)[0]
                .contiguous()
                .unsqueeze(1)
            )  # bx1xm
            DXYf[:, i : (i + 1), :] = (
    ...
    try:
        ...
        for i in range(
            ...
            try:
                ...
                Dy_at_x[i, ...] = torch.linalg.solve(DYYf[i, ...], -1.0 * DXYf[i, ...])
        ...
        with torch.enable_grad():
            ...
            def closure():
                if torch.is_grad_enabled():
                    opt.zero_grad()
                error = weightedAngularReprojectionError(
                    W, p2d, p3d, theta
